...
void drawHistograms() {
  ...
  TH1F *histo1 = (TH1F*)file->Get("dataset/InputVariables_Id/tau4__Signal_Id");
  TH1F *histo2 = (TH1F*)file->Get("dataset/InputVariables_Id/tau4__Background_Id");
  ...
  histo1->GetXaxis()->SetTitle("#tau_{4}");
  histo1->GetYaxis()->SetTitle("Relative occurence");
  histo2->GetXaxis()->SetTitle("#tau_{4}");
  histo2->GetYaxis()->SetTitle("Relative occurence");

  // Set custom histogram title
  histo1->SetTitle("Fat jet #tau_{4}");
  histo2->SetTitle("Fat jet #tau_{4}");
  ...
  canvas->SaveAs("top_tau4_distribution.pdf");
