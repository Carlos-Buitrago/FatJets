...
\rhead{Carlos Buitrago} %%Right header
...
\usepackage[square]{natbib} %%Or change 'round' to 'square' for square backers
% \setcitestyle{aysep={}}
...
\def\biblio{\bibliographystyle{unsrtnat}\bibliography{\main/Thesis_bib}}
...
\subfile{Chapters/Theory.tex}
  
\subfile{Chapters/Datasets.tex}
...
\bibliographystyle{unsrtnat} %%bib style found in bst folder, in bibtex folder, in texmf folder.
%\setlength{\bibsep}{0.5pt} %%Changes spacing between bib entries
\bibliography{Thesis_bib} %%bib database found in bib folder, in bibtex folder
